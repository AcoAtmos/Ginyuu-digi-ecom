...
exports.validatePromo = async (req, res) => {
    try {
        ...
        if (!subtotal || subtotal <= 0) {
            return res.status(400).json({ status: "error", message: "Valid subtotal is required" });
        }
        ...
        if (result.valid) {
            res.status(200).json({
                ...
                data: { code: result.code, discount_pct: result.discount_pct, discount_amount: result.discount_amount }
            });
        } else {
