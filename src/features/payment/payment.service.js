...
exports.getInvoiceByNumber = async (invoice_number) => {
    ...
    try {
        const query = `
            SELECT i.id AS invoice_id, i.invoice_number, i.discount_amount, i.total, i.issued_at, i.unique_num, i.status,
                   o.id AS order_id, o.subtotal, o.payment_method, o.created_at AS order_date,
...
exports.getNotification = async (result) => {
    try {
        return { code: 200, status: "success", message: "Success", data: result.payload?.notification };
    } catch (err) {
        console.error("getNotification error:", err);
        return { code: 500, status: "failed", message: "Get Notification Failed", data: null };
    }
};

exports.getInvoiceByTotal = async (result) => {
    try {
        return { code: 200, status: "success", message: "Success", data: result };
    } catch (err) {
        console.error("getNotification error:", err);
        return { code: 500, status: "failed", message: "Get Invoice By Total Failed", data: null };
    }
};
...
exports.verifyPayment = async (result) => {
    try {
        ...
        return { code: 500, status: "failed", message: "Verify Payment Failed", data: null };
    }
};

exports.createResponse = async (result) => {
    return { code: result.code, status: result.status, message: result.message, data: result.data };
};
